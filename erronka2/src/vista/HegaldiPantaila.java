...
public class HegaldiPantaila extends JFrame {
	 ...
	 */
	public static void pantViajes(Connection conexioa) {
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					HegaldiPantaila frame = new HegaldiPantaila(conexioa);
	 ...
	 * Create the frame.
	 */
	public HegaldiPantaila(Connection conexioa) {
		...
		try {
			String sql = "SELECT Izena, bidai_mota_kod, hasiera_data, amaiera_data, herrialde_kod FROM bidaia INNER JOIN agentzia USING(agentzia_id) WHERE agentzia_id=001";
